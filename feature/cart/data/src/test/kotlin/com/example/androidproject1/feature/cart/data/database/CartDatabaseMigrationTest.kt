...
private const val DATABASE_NAME = "cart-migration-test.db"

/**
 * Every committed schema of the cart database migrates into the current one.
 *
 * This is the half a DAO test cannot reach. `CartDatabaseTest` builds the current schema from
 * scratch, which is what a fresh install does and what every test machine does; a user who already
 * has the app runs the migration instead, and until now nothing ran one at all. The first schema
 * change is where somebody's cart is quietly lost.
 *
...
class CartDatabaseMigrationTest {
    ...
    private val committedVersions: List<Int>
        get() {
            val directory = requireNotNull(CartDatabase::class.qualifiedName)
