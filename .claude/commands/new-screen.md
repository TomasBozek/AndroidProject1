...
`--with-args 'productId:String,rating:Int'`. It generates the `@Serializable data class` route, a
ViewModel taking a `SavedStateHandle` and reading `navArgs<XDestination>()`, and a
Robolectric-annotated test. Never hand-convert a `data object` route into a `data class`: doing that
is what produced a screen loading from a `LaunchedEffect` instead of its `SavedStateHandle`, which
