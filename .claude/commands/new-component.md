...
`--feature` when it belongs to one feature and should not be shared — it lands in that module's
`presentation/component/`, one file per composable, never beside a screen. `--state` adds an
`@Immutable` state class with the `PREVIEW` fixture `doctor.py` requires.
