...
Start `$ARGUMENTS`. The rules are `docs/work/PROCESS.md`; read it once per session, not once per
task.

1. `git fetch origin && git switch main && git pull --ff-only`.
2. Open the plan under `docs/work/plans/` whose header says `Status: open`. Find the board line and
...
6. If the task has a **Decide first** line, settle it first and write the row into
   `docs/spec/DECISIONS.md` under the number the plan pre-assigned. That row is part of this commit.
...
- A fact you changed moves to its one doc in this same commit — `docs/work/PROCESS.md` § Which doc
