...
| `network/NetworkEngine` (`src/dev`) | the fixture `MockEngine` (D20): the catalog routes under `BASE_URL`, and under `api.themoviedb.org` (D80) `movie/popular?page=n` from `res/raw/fixture_movies_page_{1,2,3}.json` — twenty rows each, TMDB's shape, a `422` past page 3 the way TMDB answers — and `movie/<id>` as the row merged with `fixture_movie_details.json`. Host first, then path, so two servers do not share a route table. While `FixtureNetwork.realApi` is set and the build has a key, a TMDB request goes through a bare `HttpClient` over `cachedOkHttpEngine` and its answer is returned as the mock's (D81); the catalog never leaves. `NetworkEngineTest` drives it through a real client, the real half a `MockEngine` of its own |
