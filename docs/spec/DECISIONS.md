...
| D41 | Documentation system | Five zones — spec, reference, guides, work, archive — indexed by `docs/README.md`, with `CLAUDE.md` holding the rules alone. Work is planned as releases of point-estimated tasks with four-character ids, and checks are tiered T0–T4. Replaces D37's single plan file |
| D42 | The 2026-09 retrospective | Archived under `docs/archive/2026-09-review/` and deleted when release A ships. A task that came from it says so once, as `(was F12)` |
